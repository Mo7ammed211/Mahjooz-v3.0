...
(function () {
  ...
  window.adminAssignDriver = function (orderId) {
    ...
    /* رتّب: المندوبون الذين لديهم مسافة أولاً (الأقرب للمزود)، ثم الباقون */
    drivers.sort((a, b) => {
  ...
  function _driverRow(d, order, currentUid, idx) {
    ...
    return `
    <div style="background:var(--bg-card);border:1.5px solid ${isAssigned ? 'rgba(16,185,129,0.4)' : isNearest ? 'rgba(124,58,237,0.35)' : 'var(--border)'};border-radius:14px;padding:14px 16px;display:flex;align-items:center;gap:12px;transition:all 0.15s;position:relative"
         onmouseover="this.style.borderColor='var(--primary)';this.style.background='rgba(124,58,237,0.04)'"
         onmouseout="this.style.borderColor='${isAssigned ? 'rgba(16,185,129,0.4)' : isNearest ? 'rgba(124,58,237,0.35)' : 'var(--border)'}';this.style.background='var(--bg-card)'">

      ${isNearest ? `<div style="position:absolute;top:-1px;right:12px;background:linear-gradient(135deg,#7c3aed,#0d9488);color:#fff;font-size:10px;font-weight:900;padding:2px 10px;border-radius:0 0 8px 8px">⚡ الأقرب للمزود</div>` : ''}

      <!-- صورة/حرف -->
      <div style="position:relative;flex-shrink:0;${isNearest?'margin-top:8px':''}">
      ...
      <!-- بيانات -->
      <div style="flex:1;min-width:0;${isNearest?'margin-top:8px':''}">
        <div style="font-weight:900;font-size:14px;color:var(--text-main);display:flex;align-items:center;gap:6px;flex-wrap:wrap">
          ${esc(d.name||'مندوب')}
